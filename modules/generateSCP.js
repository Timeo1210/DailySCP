...
async function generateSCP() {
  ...
  let SCP = {};
  const backupFileStream = FileAPI.createWriteStreamFromFile(
    process.env.BACKUP_DATABASE_FILEPATH,
    "a"
  );
  while (isSCPValid === false) {
    const SCPNumber = await generateSCPNumber();
    console.log("Trying with :", SCPNumber);
...
async function getSCPImage(imgURL) {
  try {
    ...
    FileAPI.writeToFile(
      process.env.NEXTSCP_IMAGE_BIN_FILEPATH,
      FileAPI.readFromFile(process.env.NEXTSCP_IMAGE_FILEPATH)
    );
    return "BIN";
  } catch (e) {
    return "";
...
async function generateSCPNumber() {
  try {
    ...
    while (newSCPNumber === null) {
      ...
      const queryDatabase = await DatabaseAPI.getSCPFromNumber(
        randomStringNumber
      );
...
function checkIfSCPIsValid(SCP = null) {
  try {
    if (
      typeof SCP.title === "string" &&
      SCP.title.length > 1 &&
      typeof SCP.class === "string" &&
      SCP.class.length > 1
    )
      return true;
  } catch (e) {
